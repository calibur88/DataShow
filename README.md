...
| 目录 | 定位 | 插件 id | DSQL |
|---|---|---|---|
| [`data_show/`](data_show/) | **正式版**（当前 2.0.0）：清理后的插件工程，只含源码、构建配置与用户文档 | `data-show` | v2.0 |
| [`data_show_test/`](data_show_test/) | **草稿开发目录**：新功能、DSQL 语法演进、测试与规范文档都在这里进行 | `datashow-dev` | v2.0 |

两者 id 不同，可在同一 vault 中共存。**版本规则**：

- 版本号三段式 `x.y.zzz`：正式版末段为 `0`（如 `2.0.0`），无测试后缀；
- **测试版**在 `data_show_test/` 中开发，`package.json` 版本号与正式版对齐（不随测试递增），
  开发条目以 CHANGELOG 末段编号递增（如 `2.0.001`）；
...
- **数据可编辑**：卡片视图单击字段值直接改 frontmatter（派生列只读），保存后结果自动刷新；
- **DSQL v2.0**：标记语法（`**SELECT**` / `%==%`），子句按前件关系书写（SELECT 可省略），
  完整表达式、多级排序与自定义优先级，TOTAL 全表聚合与 `$变量$` 派生，
  三种「无」语义分家（正常值 / 空容器 / 未赋值），视图三关键词
  `**TABLE_VIEW**` / `**LIST_VIEW**` / `**CARD_VIEW**`（旧 `TABLE` / `LIST` 已废除），
  规范见 [`data_show/docs/DSQL-EBNF.md`](data_show/docs/DSQL-EBNF.md)；
- **索引**：Obsidian metadataCache 增量索引，frontmatter 原样入行（不解释、不改写业务字段）；
- **视图**：表格 / 列表 / 卡片看板（日历、统计为规划项）。
