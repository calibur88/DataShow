...
- **DSQL 语言版本**：`2.6`（v2.6 新增**域扩展**：块 `{ }` 语法声明多个子域，`<域>` / `<域>::字段` 引用子域，
  `**YIELD**` + `**IN**` / `**DIFF**` 建立跨数据集的逐行关系并按行展开；**无块的查询完全走 v2.5 旧算法**、
  输出逐字节一致；
  v2.5 修复两池隔离：`$变量$` 列标签不再与裸标识符列标签跨池判重，列标签唯一性按两池分别判定；
...
> **版本兼容性**：
> - **插件 `2.3.0`**：新增域扩展（DSQL 2.6）与配套演示数据 / 七个「域示例」看板；域扩展的输出显示口径
>   单独定稿（列标签 `<域>` 保留尖括号、`$槽位$` 去 `$`；域对象 `[字段:值]`；槽位 `true`/`false`
>   与 `,` 无空格），**只作用于域扩展输出**，其余查询显示零变化；内部把表达式求值与 FROM 判定下沉为
>   `dsql/expr.ts`、`dsql/source.ts`，域扩展执行独立为 `dsql/domains.ts`（纯重构，旧算法行为零变化），
>   已有 `data.json` 可继续使用；
...
>   `$槽位$`——**破坏性**；
> - **DSQL `2.2`**：新增 `**SEARCH**` 正文抽取子句；比较语义修订为「同能转数字 → 数值比」——**破坏性**。
