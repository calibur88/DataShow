...
> **DSQL 语言版本**：`2.6`（新增**域扩展**——块 `{ }` 语法与 `<域>` / `::` / `**YIELD**` / `**IN**` /
> `**DIFF**` 四条新标记，在旧算法之上叠加一层「多子域声明 → 跨域引用 → 行展开」模型；
> **无块的查询完全走 v2.5 旧算法**，输出与 v2.5 逐字节一致。
> 历史语言版本说明见 [CHANGELOG.md](../CHANGELOG.md)）
> **文档版本**：`2.6.1`（语言版本与插件版本各自独立；本次修订：域扩展**输出显示口径**定稿——
> 列标签去 `$`、域对象 `[字段:值]`、槽位 `true`/`false` 与 `,` 无空格，见 §6.13.7。
> 历史文档版本见 [CHANGELOG.md](../CHANGELOG.md)）
>
