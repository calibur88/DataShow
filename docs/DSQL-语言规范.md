...
> **DSQL 语言版本**：`2.6`（v2.6 新增**域扩展**——块 `{ }` 语法与 `<域>` / `::` / `**YIELD**` / `**IN**` /
> `**DIFF**` 四条新标记，在旧算法之上叠加一层「多子域声明 → 跨域引用 → 行展开」模型；
> **无块的查询完全走 v2.5 旧算法**，输出与 v2.5 逐字节一致；
> v2.5 修复两池隔离——`$变量$` 列标签不再与裸标识符列标签跨池判重，唯一性按两池分别判定；
...
> **文档版本**：`2.6.1`（语言版本与插件版本各自独立；2.3.1 定稿：别名两分法、LIMIT 整数、一元正负号语义；
> 2.4.0：WHILE 子句、SEARCH 双向绑定、TOTAL 对数字串的隐式转换、WHILE 边界字面量收紧；
> 2.5.0：两池隔离下列标签唯一性改为按池判定，跨池同名不再误报；
> 2.6.0：域扩展（§6.13）——域标记词法硬约束、块与子域语法、作用域链与两条查找路径、
> IN / DIFF 槽位律表与行展开基数；2.6.1：域扩展**输出显示口径**定稿——列标签去 `$`、
> 域对象 `[字段:值]`、槽位 `true`/`false` 与 `,` 无空格，见 §6.13.7）
>
...
| 属性 | 说明 |
|---|---|
| **标记体系（字面语法）** | 关键词 / 函数：`**WORD**` 包裹 · 运算符：`%op%` 包裹 · 字符串：`'...'` · 路径：`"..."` · 字段：裸标识符 · 域：`<域>` · 跨域引用：`<域>::字段` |
| **两套算法** | **无块**（无 `{ }`）→ DSQL 2.5 旧算法（唯一入口，与旧版逐字节一致）；**有块** → 域扩展算法（§6.13），块内子查询仍走旧算法 |
| **子句顺序（前件关系）** | 书写顺序自由，每条子句至多一次；**WHERE / COUNT / SORT / LIMIT / WHILE 以 `**FROM**` 为前件**，**SEARCH 以 `**WHILE**` 为前件**（两者必须同时出现）；`**SELECT**` 可省略（默认 `*`）。**域扩展算法另有自己的子句集**（顶层 `**SELECT** <域>` + 块 `{ }`），见 §6.13 |
| **执行顺序** | FROM 源解析 → [ext] 行并入 → WHILE 驱动 SEARCH 结构匹配 → 聚合遍（TOTAL）→ WHERE 行过滤 → COUNT 分类计数 → SORT 排序 → LIMIT 截断 → SELECT 投影（与书写顺序无关，由 AST 固定）。域扩展算法：子域绑定求值 → `**YIELD**` 展开 → 行展开 → 顶层投影 |
| **数据中立** | 不预设字段；UTF-8 字节序确定性排序 |
| **非致命语义** | 类型不匹配 / 除零 / 字段缺失求值为 null，不中断查询，计入 `warnings` |

**关键词（`**` 包裹，约定全大写；下表即全部，共 24 个）**：
...
(* ---------- 域标记（DSQL 2.6） ---------- *)
DOMAIN      = "<" , IDENT , ">" ;         (* 与 **..** / %..% / '..' / ".." / $..$ / [..] 并列的独立名字空间 *)
CROSS_REF   = DOMAIN , "::" , IDENT ;     (* 跨域引用，只允许一级 *)
BLOCK_OPEN  = "{" ;
BLOCK_CLOSE = "}" ;
(* 上述四者**全部由词法层拦截**（不得下放给 parser）：
   · `<` 与 `>` 之间只允许一个 IDENT（含空白 / 嵌套 <> 一律词法错误）；
   · `<域>::<域>`         → :: 右侧非 IDENT，词法错误；
   · `<域>::<域>::字段`   → 第一个 :: 的右侧已是 <域>，同上一条拦截（引用链只允许一级）；
   · `name::field`        → :: 左侧非 DOMAIN，词法错误；
   · 块 `{ }` 未闭合 / 多余 `}` → 词法错误。
   域标记不跨行（与 **WORD** 同口径）；裸标识符 `domain` / 字符串 `'<人物>'` 不受影响（词法隔离）。 *)
```
...
```ebnf
query          = flat_query | domain_query ;               (* 无块 → 旧算法；有块 → 域扩展算法（§6.13） *)

flat_query     = [ view ] , { clause } , EOF ;             (* 各 clause 至多一次；WHERE/COUNT/SORT/LIMIT/WHILE 要求 FROM 已出现，SEARCH 要求 WHILE 已出现；
...
(* ---------- 域扩展（DSQL 2.6；语法单元与上式互斥，由是否有 block 分流） ---------- *)
domain_query   = [ view ] , top_select , block , EOF ;
top_select     = **SELECT** , top_item , { "," , top_item } ;   (* 顶层只接受单级 <域> / $槽位$；不接受 <域>::字段（不可传递性，§6.13） *)
top_item       = DOMAIN | variable ;

block          = "{" , { block_item } , "}" ;
block_item     = subdomain | yield_clause ;
subdomain      = "{" , sub_select , { subdomain } , "}" , **AS** , DOMAIN ;  (* **AS** 必填：无 **AS** 即非子域 *)
sub_select     = **SELECT** , select_list ,
                 **FROM** , source ,
                 [ **WHERE** , expr ] ;                       (* **FROM** 必填；子查询内仅这三个子句 *)
select_list    = select_field , { "," , select_field } ;
select_field   = IDENT | DOMAIN ;                             (* 裸字段 或 单级 <域>（域引用，取值见 §6.13） *)

yield_clause   = **YIELD** , yield_item , { "," , yield_item } ;   (* 同层至多一次；前件 = 同层已声明子域 *)
yield_item     = cross_ref , yield_op , cross_ref , [ **AS** , variable ] ;
yield_op       = **IN** | **DIFF** ;
cross_ref      = DOMAIN , "::" , IDENT ;

variable       = "$" , ident , "$" ;                          (* 域扩展中：顶层 SELECT 的槽位声明，或 YIELD 项的填充名 *)
```
...
- **分流规则**：查询内出现 `{ }` 块，或顶层 `**SELECT**` 列表中出现过 `<域>` → 走域扩展算法
  （后者即「有 `<域>` 而无块」，仍报语义错误「域 <x> 未声明」）；否则一律走旧算法，输出与 v2.5 逐字节一致；
- `block` 只出现在顶层；`sub_select` 的 `**FROM**` 必填（唯一数据来源）；
- `subdomain` 的 `**AS** <域>` 必填，省略即非子域；
- **`**AS**` 是硬约束**：无 `**AS**` 即无名字——域名（子域）、变量（`**YIELD**` 项）、裸标识符列标签（旧算法）均如此；
- **`**YIELD**` 只出现在顶层块**：子域块内只有 `sub_select` + 嵌套子域（`**YIELD**` 它处一律语法错误）。
...
> **域扩展算法不适用本表**：有块时子句集换成 `top_select` + `block`（§3 / §6.13），
> 顶层没有 `**FROM**`（数据源在子查询内），块内只有 `**SELECT**` / `**FROM**` / `**WHERE**`
> 与 `**YIELD**`（前件 = 同层已声明子域）。`**YIELD**` / `**IN**` / `**DIFF**` 出现在旧算法区间内，
> 一律按「前件缺失」致命报错。
...
> `**IN**` / `**DIFF**` **不是运算符**，而是 `**YIELD**` 项的连接符（§6.13）——它们不参与表达式的
> 优先级体系，也不能出现在 `**WHERE**` / `**SORT**` / `**SELECT**` 表达式内（出现即致命）。
> `%==%` 等比较运算符照旧；`**IN**` 的成员判断口径见 §6.13（与 §6.3 同源）。
...
域扩展在旧算法之上叠加一层**多数据集模型**：块 `{ }` 内声明若干**子域**（各自一条子查询），
子域之间用 `**YIELD**` 的 `**IN**` / `**DIFF**` 建立**逐行关系**，最后按行展开投影。
它回答的是旧算法回答不了的问题——「两份互不相干的笔记集合之间，谁引用了谁 / 谁还有谁没有」。

**两条算法并存**：

| 条件 | 算法 | 兼容性 |
|---|---|---|
| 无块（无 `<域>`、无 `{ }`） | DSQL 2.5 旧算法（唯一入口） | 输出与 v2.5 **逐字节一致** |
| 有块（或顶层 `**SELECT**` 出现 `<域>`） | 域扩展算法 | 新语法，块内子查询仍走旧算法的最小子集 |

#### 6.13.1 名字空间与词法硬约束

`<域>` 与 `**WORD**` / `%op%` / `'...'` / `"..."` / `$变量$` / `[...]` 并列，是**独立名字空间**：
裸名池（frontmatter / `file.*` / SEARCH 别名 / 裸列标签）与域名池语法层即分离，同名互不干扰。
`<` / `>` / `{` / `}` / `**YIELD**` / `**IN**` / `**DIFF**` 在 v2.5 中均无用途，故无冲突。

四条硬约束**全部在词法层拦截**（不得下放给 parser）：

| 写法 | 判定 | 报错 |
|---|---|---|
| `<域>::<域>` | 词法 | `:: 右侧须 IDENT，<域> 非法（跨域引用只允许一级 <域>::字段）` |
| `<域>::<域>::字段` | 词法 | 同上（**第一个** `::` 即被拦截，引用链只允许一级） |
| `裸名::字段` | 词法 | `:: 左侧须为域标记（如 <人物>::名字）` |
| `<` 内非单个 IDENT（含空白 / 嵌套 `<>`） | 词法 | `域标记 <…> 内应为单个标识符（如 <人物>）` |
| `<` 未闭合 / 跨行 | 词法 | `域标记未闭合（需以 > 结束，如 <人物>）` / `域标记不能跨行` |
| 块 `{ }` 未闭合 / 多余 `}` | 词法 | `块 { 未闭合（需与 } 配对）` / `多余的 }（块括号 { } 不配对）` |

> `**IN**` / `**DIFF**` / `**YIELD**` 加入 `**WORD**` 关键词表后，裸标识符 `in` / `diff` / `yield`、
> `$in$`、字符串 `'**IN**'` 均不受影响（词法隔离，与 `WHILE` 同口径）。

#### 6.13.2 产生完成即见律（核心）

```text
一个名字/值在其产生完成的时点之后才可见。
```

| 名字 | 产生完成时点 | 可见范围 |
|---|---|---|
| 行字段（裸名） | 子查询 FROM 建行完成 | 该子查询的 WHERE + SELECT |
| `<域>` | 子域块闭合（`} **AS** <x>`） | 闭合点之后的同层 `**YIELD**` 项 + 外层 SELECT |
| `$槽位$`（`**YIELD**` 填充） | 行展开阶段逐行求值完成 | 顶层 `**SELECT**`（同一行内） |

#### 6.13.3 SELECT 宽松 / YIELD 严格

不是两条规则，是同一律在两种**扫描位置**的表现：

```text
YIELD  可见 <x> ⇔ <x> 已在作用域链内闭合 ∧ <x> 的闭合点在文本上位于该 YIELD 之前
SELECT 可见 <x> ⇔ <x> 已在作用域链内闭合（不限文本位置）
```

**理由**：`**YIELD**` 在展开阶段**扫描中途**执行，只见已闭合子域；`SELECT` 在**扫描完成后**执行，见全部。
改查找模式 = 改执行模型。

**作用域链**（同名域在不同层各为独立绑定，内层不遮蔽外层）：

| 查找位置 | 可见作用域 | 文本序约束 |
|---|---|---|
| 顶层 `**SELECT**` 的 `<域>` | 根块作用域 | 无（宽松） |
| 子域 `sub_select` 的 `<域>` | 自身嵌套子域 → 声明它的作用域 → 逐层向外 | 无（宽松） |
| `**YIELD**` 项的 `<域>` | 所在块作用域 → 逐层向外 | **同层须写在前面**（严格）；更外层天然满足 |

#### 6.13.4 不可传递性

**根因：循环依赖。** 顶层 `**SELECT**` 若允许 `<人物>::<武器>`，就会「顶层声明依赖内层结构，
而内层结构嵌套在顶层声明之下」→ 解析顺序崩溃。∴ 顶层 `select_item = DOMAIN | variable`，只接受单级名字。
由此推出三条：

1. `::` 只允许一级 `<域>::字段`（词法层拦截）；
2. 内层子域不向外暴露、不自动提升（外层引用内层 → 致命「不向外暴露」）；
3. 同名域在不同层各为独立绑定（`<人物>` 块内的 `<武器>` 与外层 `<武器>` 互不影响）。

#### 6.13.5 执行模型

```text
子域绑定求值（拓扑序）→ YIELD 展开（定 ROW / AGG，算出行数）→ 行展开（笛卡尔积）→ 顶层投影
```

- **子域绑定求值**：每个子域 = `FROM 命中 → WHERE 过滤 → 逐行投影对象`；
  子查询 SELECT 中的 `<域>` 是**域引用**——取被引用域的**全量对象**（单对象给对象本身、多对象给对象序列）；
  绑定顺序按**依赖拓扑序**（SELECT 前向引用不构成文本序约束），循环依赖 → 致命；
- **`**YIELD**` 展开**：逐项求值，写回该行变量环境（槽位裸名）；
- **行展开**：`rowDomains` 的笛卡尔积，逐行推进（最后一维最快），无 `**YIELD**` 时 = 根块全部子域；
- **顶层投影**：`<域>` 取当前行对象、`$槽位$` 取该行 `**YIELD**` 值（未被任何 `**YIELD**` 填充的
  槽位**静默忽略**——与旧算法裸槽位的未填充口径一致）。

#### 6.13.6 IN / DIFF 语义

**AGG / ROW 只由运算符槽位决定，与字段来自哪个域无关**：

```text
IN(a, b)   ：单射拆分 a，逐元素判断 a_i ∈ b，逐行输出布尔。槽位 (ROW, ROW) → ROW
DIFF(a, b) ：左参取全集，逐行取右参，输出 a \ b，保留 a 顺序。槽位 (AGG, ROW) → AGG→ROW
```

**律表**（只作用于 `**IN**` / `**DIFF**` 项组合）：实现即 `isRow(a) || isRow(b) ? ROW : AGG_ROW`。
...
```text
阶段 1（AGG）：所有 AGG 槽位取值（DIFF 左参），多重独立求值。
阶段 2（ROW）：所有 ROW 槽位涉及域做笛卡尔积展开，逐行求值。
行数 = 所有 ROW 槽位涉及的不同域行数之积（同域只计一次）。
无 YIELD → 所有根块子域 ROW（笛卡尔积）。
```

| 项 | ROW 位涉及域 |
|---|---|
| `a **IN** b` | `a` 的域 ∪ `b` 的域 |
| `a **DIFF** b` | 仅 `b` 的域（左参 `a` 取全集，不贡献行） |

**取值口径**：

| 槽位 | 取值 |
|---|---|
| ROW | 当前行元组下该域对象的字段值；域不参与展开或行越界 → `null` |
| AGG（`**DIFF**` 左参） | 该域**全量对象**上该字段的取值序列；字段值为数组时**按元素摊开**；`null` / 缺失不入序列 |
| `a **IN** b` | `a` 为数组 → 任一元素命中即 true；`b` 为数组 → 集合成员；`b` 非数组 → 相等判断；`b` 为 `null` → `false` |
| `a **DIFF** b` | `a` 全集逐个剔除 `b` 中的值（`b` 为数组则整体剔除），**保留 `a` 的顺序** |

相等判断与 §6.3 比较口径**同源**（`coerce.ts` 单一事实源）：同能转数字 → 数值比，同不转 → UTF-8 字节序比。

#### 6.13.7 顶层投影与显示口径

**输出列标签**（§1 的类型标记 / 命名空间标记之分）：

| 名字来源 | 语言写法 | 输出列标签 | 规则 |
|---|---|---|---|
| 域 | `<人物>` | `<人物>`（保留尖括号） | `<...>` 是**类型标记** → 保留 |
| 变量槽位 | `$未引用$` | `未引用`（去 `$`） | `$...$` 是**命名空间标记** → 输出层去掉 |
| 裸标识符列标签（旧算法） | `**AS** 任务` | `任务`（不变） | — |

> 去 `$` **只在输出层生效**：语言层引用仍须写 `$变量$`；`<域>` 的尖括号在语言层与输出层语义一致
> （均表示域标记），不做任何转换。表头（表格 / 列表 / 卡片）、CSV 表头、JSON 键、结果区搜索文本
> 共用同一份输出列标签。
...
| 项 | 口径 |
|---|---|
| `<域>` 列取值 | 该域**参与行展开** → 当前行对象；不参与 → 全量对象（单对象给对象、多对象给序列、空域 `null`） |
| `$槽位$` 列取值 | 该行 `**YIELD**` 值；未被任何 `**YIELD**` 填充 → 不成为列（静默忽略） |
| **域对象显示** | `[字段名:值][字段名:值]…`——每字段独立 `[...]` 包裹、`名:值` 冒号**无空格**、字段之间**紧邻无分隔符**；字段顺序 = 子域 `**SELECT**` 声明顺序 |
| **槽位值显示** | 直接输出值，**不带 `$`、不加 `[]`**：布尔 → `true` / `false`；数组 → 元素 `,` 分隔、**无空格**（如 `金针,铁针`）；标量 → 原样 |
| `null` / empty 值 | 两级同口径 → `—`（域对象字段与槽位值均然） |
| 「文件」列 | 取 `rowDomains` **首个域**当前行的来源笔记（可点击打开）；无行展开域时为空 |
| **JSON 导出** | 域对象仍输出为**结构化对象**（字段 → 值），不做 `[...]` 文本化；槽位输出**原值**（布尔为 `true` / `false`，数组为 JSON 数组）；`null` / empty → `null` 且**保留键** |
| 只读性 | 域扩展结果为合成行（`fields` 为投影值），卡片视图内联编辑对域列无效 |

**示例**（数据见 `test-vault/示例/域示例/`）：

```text
表头：<人物>                                <武器>         人物有武器
行：  [name:傻青][wolf:狼牙棒,金针]         [name:狼牙棒]   true
...
> **口径隔离**：上述布尔（`true` / `false`）与数组（`,` 无空格）口径**只作用于域扩展输出**；
> 其余查询维持既有口径（布尔 → `是` / `否`，数组 → `, ` 连接）。CSV 里域对象文本含逗号，
> 按 RFC 4180 自动加双引号（`"[name:傻青][wolf:狼牙棒,金针]"`）。

#### 6.13.8 错误处理

| 场景 | 阶段 | 判定 / 报错 |
|---|---|---|
| 顶层 `**SELECT**` 写 `<域>::字段` | 语法 | 顶层只接受单级 `<域>` / `$槽位$` |
| `<域>::<域>` / `<域>::<域>::字段` | 词法 | `::` 右侧非 IDENT |
| `裸名::字段` | 词法 | `::` 左侧非 DOMAIN |
| 块 `{ }` 未闭合 / 多余 `}` | 词法 | 未闭合 |
| `<域>` 无对应块 | 语义 | `域 <x> 未声明（缺少块 { }…）` |
| `**YIELD**` 引用未声明域名 | 语义 | 域 `<x>` 未声明 |
| `**YIELD**` 引用写在它之后的同层子域 | 语义 | `<x> 未在该 **YIELD** 之前声明` |
| `**YIELD**` / 外层 SELECT 引用内层子域 | 语义 | `<x> 不向外暴露` |
| 同名域在同层重复声明 | 语义 | `域 <x> 在同层重复声明` |
| 域引用形成循环依赖 | 语义 | `域引用存在循环依赖（<a> → <b> → <a>）` |
| 同层 `**YIELD**` 重复 | parse | `**YIELD** 子句重复出现（同层至多一次）` |
| `**YIELD**` 出现在同层无已声明子域之前 | parse | 前件缺失 |
| `**YIELD**` 出现在子域块内 | parse | 只能写在顶层块 |
| `**IN**` / `**DIFF**` 出现在 `**YIELD**` 之外 | parse | 前件缺失 |
| `**YIELD**` 项的 `**AS**` 非 `$变量$` | parse | `**AS**` 必须为 `$变量$` |
| 同层 `**YIELD**` 的 `**AS** $变量$` 重名 | parse | 槽位重复声明 |
| `sub_select` 缺 `**FROM**` | parse | `**FROM**` 是唯一数据来源 |
| 子查询内出现其它子句 | parse | 只支持 `**SELECT**` / `**FROM**` / `**WHERE**` |
| 块之后仍有子句 | parse | 多余的查询子句 |

#### 6.13.9 示例（可逐条执行核验）

```sql
-- 1. 单子域 + 多子域并列（无 YIELD → 所有子域 ROW → 笛卡尔积 |人物| × |武器|）
**TABLE_VIEW** **SELECT** <人物>, <武器>
{
  { **SELECT** name, wolf **FROM** "示例/域示例/人物" } **AS** <人物>
  { **SELECT** name **FROM** "示例/域示例/武器" } **AS** <武器>
}

-- 2. IN：单射拆分逐元素判断（槽位 (ROW, ROW) → ROW）
**TABLE_VIEW** **SELECT** <人物>, <武器>, $人物有武器$
{
  { **SELECT** name, wolf **FROM** "示例/域示例/人物" **WHERE** type %==% '人物' } **AS** <人物>
  { **SELECT** name **FROM** "示例/域示例/武器" **WHERE** type %==% '武器' } **AS** <武器>
  **YIELD**
    <武器>::name **IN** <人物>::wolf **AS** $人物有武器$
}
-- 实测 6 行；列标签 <人物> | <武器> | 人物有武器；人物有武器 = true/true/false/true/false/false

-- 3. DIFF：左参取全集、右参逐行（槽位 (AGG, ROW)；行数 = |人物|，AGG 不贡献行）
...
-- 实测 2 行：[name:傻青][wolf:狼牙棒,金针] | 铁针；[name:王五][wolf:狼牙棒] | 金针,铁针（保留武器全集顺序）

-- 4. 混合：律表 ROW ∘ AR = ROW（行数 = |剧情| × |人物|）
**TABLE_VIEW** **SELECT** <剧情>, <人物>, $在剧情里$, $非剧情人物$
{
  { **SELECT** name, chart **FROM** "示例/域示例/剧情" **WHERE** name %==% '苍蓝传' } **AS** <剧情>
  { **SELECT** name **FROM** "示例/域示例/人物" **WHERE** type %==% '人物' } **AS** <人物>
  **YIELD**
    <人物>::name **IN** <剧情>::chart      **AS** $在剧情里$,
    <剧情>::chart **DIFF** <人物>::name    **AS** $非剧情人物$
}
-- 实测 2 行：傻青 → true | 笨蛋；王五 → false | 傻青,笨蛋

-- 5. YIELD 缺省 **AS**：计算照常执行，但不产生 $槽位$、不投影
**SELECT** <a>
{
...
#### 6.13.10 实现口径声明（与设计草案的差异）

本节为 v2.6 首次落地时的**实现决策记录**，凡与上游设计草案不一致处均在此报备：

| # | 项 | 落定口径 | 理由 |
|---|---|---|---|
| 1 | 无块但含 `<域>` | 仍走**域算法**并报「未声明域」（不落回旧算法） | 旧算法无 `<域>` 原子，落回只会得到「应为表达式」，语义错误信息更准 |
| 2 | 块内子查询的可用子句 | 仅 `**SELECT**` / `**FROM**` / `**WHERE**` | 遵循 EBNF；`**SORT**` / `**LIMIT**` / `**WHILE**` / `**SEARCH**` / `**COUNT**` / `**WITHOUT** **ID**` 一律致命 |
| 3 | 顶层视图关键词 | 允许（`**TABLE_VIEW**` / `**LIST_VIEW**` / `**CARD_VIEW**`，缺省 `TABLE_VIEW`） | 与全语言一致；其余顶层子句（含 `**WITHOUT** **ID**`）不开放 |
| 4 | 子域对象对数组字段的 AGG 摊开 | `**DIFF**` 左参把数组字段按元素摊开后再取差集 | 否则 `<剧情>::chart **DIFF** <人物>::name` 会得到嵌套数组，与示例 3.2 / 4.2 的预期输出不符 |
| 5 | 域引用的取值形态 | 单对象 → 对象本身；多对象 → 对象序列；空域 → `null` | 避免「处处包一层数组」，同时保留多行语义 |
| 6 | 域对象显示 | `[字段:值][字段:值]`（字段间紧邻、`名:值` 冒号无空格） | 设计草案中的 `<人物>` 单元格写作 `傻青`、`<剧情>` 写作 `苍蓝传 chart=[…]` 属示意；实现取统一口径 |
| 6b | 布尔 / 数组的域口径**只作用于域扩展输出** | 域扩展单元格 → `true` / `false`、`,` 无空格；其余查询维持既有口径（`是` / `否`、`, ` 连接） | `formatCell` 是「值 → 文本」唯一出口但看不到列来源：域对象靠**值形态**（唯一对象形态）辨认，`$槽位$` 值由执行层包 `DomainSlotValue` 标记辨认（JSON 导出解包为原值）；既有看板显示零变化 |
| 7 | 行展开顺序 | 按域**声明序**（`id` 升序）笛卡尔积，最后一维变化最快 | 确定性输出，便于测试与验收 |
| 8 | 「文件」列来源 | `rowDomains` 首个域当前行的来源笔记 | 合成行本无 path；保留点击打开能力 |
| 9 | 嵌套块内的 `**YIELD**` | 致命（只允许顶层块） | 其填充的 `$槽位$` 无处输出（顶层 SELECT 看不到内层） |
| 10 | 同层同名域 | 致命（不同层同名合法） | 同层同名无法消歧，绑定表会静默覆盖 |
| 11 | 域引用循环依赖 | 致命（带环路径） | 该情形无任何可解语义 |
| 12 | 域模式下的 `[ext]` | 子查询 WHERE 内的 `[ext]` 仅做**行级过滤**，不触发非 md 文件级读取 | 读取范围收集以顶层 FROM 为界，域扩展无顶层 FROM；需要非 md 时请用旧算法查询 |
| 13 | 域行的 TOTAL / COUNT | 不支持（子查询内无这两个子句） | EBNF 未定义跨域聚合；跨域计数由 `**IN**` 逐行布尔 + 视图层计数覆盖 |
...
-- 13. 域扩展：跨数据集逐行关系（DSQL 2.6，有块 → 域算法，§6.13）
**TABLE_VIEW** **SELECT** <剧情>, <人物>, <武器>,
       $人物武器绑定$, $剧情人物$, $未引用$
{
  {
    **SELECT** name, chart
...
-- 行展开：{剧情, 人物, 武器} 的 ROW 位 → 1 × 2 × 3 = 6 行

-- 14. 域扩展：SELECT 宽松 / YIELD 严格（<a> 的 SELECT 前向引用 <b> 通过；YIELD 前向引用 <b> 致命）
**SELECT** <a>
{
...
-- 报错：<b> 未在该 **YIELD** 之前声明（**YIELD** 只见其之前已闭合的同层子域）
```
