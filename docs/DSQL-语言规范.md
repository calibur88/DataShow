...
> **DSQL 语言版本**：`2.5`（v2.5 修复两池隔离——`$变量$` 列标签不再与裸标识符列标签跨池判重，唯一性按两池分别判定；
> v2.4 新增 `**WHILE**` 循环驱动子句，`**SEARCH**` 改为由 `**WHILE**` 驱动迭代，
...
> **文档版本**：`2.5.0`（语言版本与插件版本各自独立；2.3.1 定稿：别名两分法、LIMIT 整数、一元正负号语义；
> 2.4.0：WHILE 子句、SEARCH 双向绑定、TOTAL 对数字串的隐式转换、WHILE 边界字面量收紧；
> 2.5.0：两池隔离下列标签唯一性改为按池判定，跨池同名不再误报）
>
...
判为致命，属跨池误判，已废除；**DSQL 2.5 起该隔离泛化到列标签**——裸 `AS x` 与 `AS $x$` 亦不跨池判重）。

**列标签与行字段的唯一性规则（致命错误，聚合遍开始前检查；判定按两池分别进行）**：

1. SELECT 内**同池**的 `**AS**` 列标签**互不相同**——裸标识符标签互不相同（行字段池，如 `AS x` 与
   `AS x`）；`$变量$` 标签互不相同（变量池，如 `AS $x$` 与 `AS $x$`）。两池之间**不互相比对**：
   裸 `AS x` 与 `AS $x$` 同名时各自独立、不报错（两池隔离）；
2. **裸标识符列标签**不得与行字段池中的名称重复——判定范围是 FROM **全量命中行的字段名并集**
   ∪ SEARCH 别名，任一行出现过该字段名即冲突（不要求所有行都有）；
   `**AS** $变量$`（以及 **TOTAL** / **COUNT** 槽位）是变量池名称，只受第 1 条（变量池内）约束，
   **不与行字段池校验**（两池隔离）。两池同名最终产出同名输出列时，由视图/导出统一以 `_N` 后缀消歧。
...
-- ✅ 两池隔离：$总成绩$ 读变量表（TOTAL 和），平均分 读行字段，同一查询各自独立（避免与裸字段同名列）
**SELECT** **TOTAL** 成绩 **AS** $总成绩$, 平均分 **FROM** "学生"

-- ✅ 两池间同名不冲突：裸 AS x（行字段池）与 AS $x$（变量池）各自独立，各成其列，不报错
**SELECT** owner **AS** x, **TOTAL** 1 **AS** $x$ **FROM** "学生"
```
