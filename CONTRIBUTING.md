...
| [CHANGELOG.md](CHANGELOG.md) | 更新日志（近期版本条目 + 归档导航；较旧版本在 `changelog-<起始版本>-<最后版本>.log`） |
...
### 3.3 文档头的版本标注（只写当前版本一条）

- 各文档头部的版本标注——`README.md` 的「插件版本 / DSQL 语言版本」、规范文档的「语言版本 / 文档版本」、
  `ARCHITECTURE.md` / `API.md` 的「当前版本」行——**只写当前版本的一条说明**，
  不得链式追加往期版本（「v2.5 修复…；v2.4 新增…」这类）；
- 版本历史（含语言版本的逐版说明、兼容性声明）一律记入**更新日志**：
  `CHANGELOG.md`（近期版本）与 `changelog-<起始版本>-<最后版本>.log`（归档），
  文档头至多在末尾留一句指向更新日志的指引；
- 版本号本身（插件版本 / 语言版本 / 文档版本）仍须在文档头如实声明，本条只约束「更新说明」的写法。
...
| `CHANGELOG.md` | 新增版本条目（按「插件更新 / DSQL 更新」两类组织，格式见 §6）；仅在**用户要求归档**时更新归档导航表与切分点（§6.3） |
...
| **测试情况** | 每个版本末尾须标注测试通过情况（如“测试：292 例（十三套件）全部通过”）。 |
| **版本兼容声明** | 若当前版本与上一版本功能兼容，在版本末尾声明（如“**本版本功能逻辑与 2.0.x 完全兼容**”）。 |
| **禁止编造** | 不确定是否重复的条目一律保留，禁止编造或删除历史条目。 |

### 6.3 主文件保留与归档划分

> **⚠️ 未经用户明确要求，不得擅自新建 / 拆分 / 重命名更新日志文件。**
> 归档切分点与文件粒度由**用户**决定：AI 与协作者不得按自己的判断"顺手"分卷、改名或另起文件。

**结构** = `CHANGELOG.md`（主文件，承载**近期**版本条目）+ 若干
`changelog-<起始版本>-<最后版本>.log`（归档文件）。

| 角色 | 内容 | 规则 |
|---|---|---|
| `CHANGELOG.md` | **近期版本条目** + 头部声明 | 头部声明须列出**全部归档文件及其覆盖区间**（导航表）+ 保留规则与警示；新版本条目一律追加到本文 |
| `changelog-<起始版本>-<最后版本>.log` | **较旧版本条目**（整段搬入） | `起始版本` = 该文件收录的**最旧**版本，`最后版本` = **最新**版本（如 `changelog-0.1.0-2.1.4.log`）；区间按**插件版本号**标注，区间内可含多条语言 / 示例条目 |

- **保留规则**：主文件保留最近 **5 条**版本条目（≈ 当前发布线与上一发布线的完整上下文）；
  条目数超过该值、或主文件超过 **400 行**时，**提示用户归档**——切分点由用户确定，不得擅自搬运；
- **归档规则**：按版本区间**整段搬入**，条目内容**一字未改**（只加文件头，不改写、不删除、不重排）；
- **格式约束沿用** [AI编写文档通用模板.md](AI编写文档通用模板.md) §6 与本节：各归档文件内同样遵守
  **版本倒序**、**条目维度**（本项目习惯为「插件更新 / DSQL 更新」两类；模板 §6.2 亦允许切换为
  「新增 / 变更 / 修复 / 废除」四类）、**兼容性声明**与**测试情况**；
- **入库要求**：`changelog-*.log` 是**必须入库**的文档（与运行期 `*.log` 不同，见 §11.4），
  不受"禁止提交 `*.log`"约束；
- **文件头**：每个 `.log` 开头写明归档性质、覆盖区间与「不得擅自新建 / 拆分 / 重命名」警示，
  并链接回 `CHANGELOG.md` 与本节。
 ...
 * @description DSQL 执行器：按 FROM → [ext] 行并入 → WHILE + SEARCH 结构匹配 → 聚合 → WHERE → COUNT → SORT → LIMIT → SELECT 管线执行查询
 */
```
...
- 套件按领域组织：功能示例 / 数学示例 / DSQL 语言 / store / 摄取层 / 索引器 / ext后缀过滤 / SEARCH正文抽取 / WHILE循环驱动 / COUNT计数 / viewSync / normalizeBoard；
- 语法 / 语义变更必须同步新增或修改 `dsql-language.test.ts` 用例（含错误路径与边界）；
- 提交前 `npm test` 全绿；测试例数变化（如「243 → 268」）同步到 README 与 ARCHITECTURE。
...
- **禁止提交的内容**：`dist/`、`test-vault-local/`、运行期 `*.log`（`changelog-*.log` 是文档，**必须入库**，见 §6.3）、`node_modules/`；
