...
- `comparison` 允许无比较符的裸操作数：此时 `concat_expr` 按**真值判断**（§6.3「真值」行：
  empty 值 / null / 0 / false / 空串 / 空数组 → 假，其余 → 真），否则 `**WHERE** **contains**(...)` 无法成立（规范 §7 示例 4 即此用法）；
...
-- ---
-- ❌ 该笔记不进入任何查询结果
-- warnings：duplicateKey（文件 "某笔记"，字段 "age"）；sourceStats/notes 档案保留 age=20 与 age=22 原始键值对
```

**摄取容错策略（骨架，供后续补充）**——摄取环节各类异常的统一口径汇总位：

- frontmatter 解析失败：占位（口径待补充）；
- 重复键：见上（剔除文件 + `duplicateKey` warnings，查询继续）；
- 类型异常（如 tags 非数组）：占位（口径待补充）。
...
| `**empty**(x)` | any → boolean | **当且仅当 x 为 empty 值（未赋值）时 → true**；其余一切输入——含 `""`、`[]`、`0`、`false`、null、缺失字段——均为 false。**empty 不处理字符串与数组，仅兜底「未赋值」一种情况** |
...
| `sourceStats` | 各叶子数据源（文件夹/标签）及其贡献行数；重复键文件的原始键值对档案（notes） |
| `where` | 过滤前/后行数 + 被剔除示例路径（最多 3 条） |
| `sort` | 排序键、方向、自定义优先级列表、比较次数 |
| `limit` | 截断前/后行数 |
| `fieldMisses` | 缺失字段名、次数、首个示例路径 |
| `warnings` | 非致命问题列表（结构化：`type` + `message` 等，含次数；类型如 除零 / 类型不匹配 / 未知函数 / **duplicateKey**（含文件名与字段名）） |
...
-- ❌ 报错：别名 '$平均分$' 与现有字段名冲突（任一命中行出现过字段「平均分」即冲突）
